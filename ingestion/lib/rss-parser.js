// rss-parser.js — minimal RSS 2.0 + Atom 1.0 parser, no dependencies.
//
// Handles the subsets actually used by MLB / ESPN / SBNation / WordPress feeds:
// RSS <item> with <title>, <link>, <description>, <pubDate>, <dc:creator>,
// <category>, <guid>, <content:encoded>, <media:content>; Atom <entry> with
// <link href="..."/>, <published>, <author><name>...</name></author>.
// CDATA sections are unwrapped. HTML is left raw — caller is responsible
// for stripping/escaping before render.
//
// Direct port of pickleball KB-0035 rss-parser.js (Session 7 reference).
// Only User-Agent string differs.

const https = require('https');
const http = require('http');
const { URL } = require('url');

const USER_AGENT = 'Mozilla/5.0 (compatible; BaseballDailyBot/1.0; +https://jjmgladden.github.io/baseball-daily/)';
...
function parseRss(xml, { sourceId, sourceName, tier, scope, feedUrl }) {
  const items = [];
...
function parseAtom(xml, { sourceId, sourceName, tier, scope, feedUrl }) {
  const items = [];
...
async function fetchFeed({ sourceId, sourceName, tier, scope, feedUrl }) {
  const xml = await fetchText(feedUrl);
  if (isAtomFeed(xml)) {
    return parseAtom(xml, { sourceId, sourceName, tier, scope, feedUrl });
  }
  return parseRss(xml, { sourceId, sourceName, tier, scope, feedUrl });
}
