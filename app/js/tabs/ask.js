// ask.js — KB-0033 AI Q&A chat tab.
...
const HISTORY = []; // [{ role: 'user'|'ai'|'error', text, ts, model?, usage? }]

async function loadConfig() {
  try { return await loadMaster('ai-config.json'); }
...
function renderHistory(rootEl) {
  ...
  if (!histEl) return;
  if (HISTORY.length === 0) {
    histEl.innerHTML = '<div class="ask-empty">Ask a question about MLB — Cardinals, Nationals, standings, today\'s schedule, recent recaps, on-this-day history, news, legends. The AI uses today\'s daily snapshot as its context.</div>';
    return;
  }
...
export async function renderAsk(root) {
  ...
  root.innerHTML =
    '<h1>Ask the AI</h1>' +
    '<div class="muted">Powered by Claude. Answers are generated from today\'s snapshot — Cardinals/Nationals games, standings, schedule, recaps, on-this-day, news, curated legends. If something isn\'t in the snapshot, the AI will say so honestly.</div>' +
    '<div id="ask-history" class="ask-history"></div>' +
    '<div class="ask-controls">' +
      '<textarea id="ask-input" rows="2" maxlength="500" placeholder="e.g. How are the Cardinals doing this season?"></textarea>' +
