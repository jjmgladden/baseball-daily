...
async function handleAi(request, env, corsHeaders) {
  ...
  try {
    const res = await fetch(contextUrl, {
      ...
      headers: { 'User-Agent': 'baseball-daily-api/1.0' }
    });
  ...
  const system = [
    'You are the AI assistant for Ozark Joe\'s Baseball Daily Intelligence Report.',
    'Answer questions about MLB baseball using ONLY the data in the supplied context bundle.',
    'If the answer is not in the context, say so honestly — do not guess or fabricate stats, dates, or biographical facts.',
    ...
    'When citing standings, recaps, or specific facts, mention the source category in parentheses (e.g., "(MLB Stats API)", "(today\'s standings)", "(yesterday\'s recap)", "(news)").',
    ...
    'The Cardinals (team ID 138) are the primary focus team; Nationals (team ID 120) are secondary.',
    'For player-specific questions, if the player is not mentioned in the context bundle, advise the user that the site\'s Players tab has a searchable directory of ~23,000 historical MLB players.'
  ].join('\n');
  ...
  const messages = [
    {
      ...
      content: [
        ...
        // call within ~5 minutes. ~5-7K tokens of curated content.
        {
          type: 'text',
          text: 'Baseball Daily context bundle (as of ' + (context.generatedAt || 'unknown') + '):\n\n' + (context.content || '(empty context)'),
...
// =====================================================================
// /submit handler — preserved from KB-0024 (dormant until Suggest UI wires up)
// =====================================================================
